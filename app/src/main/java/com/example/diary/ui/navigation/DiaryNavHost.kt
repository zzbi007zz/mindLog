...
import com.example.diary.ui.entry.editor.EntryEditorScreen
    ...
    NavHost(navController = navController, startDestination = Routes.LIST) {
        ...
        composable(
            ...
            val entryId = backStackEntry.arguments
                ?.getString(Routes.EDITOR_ARG)
                ?.toLongOrNull()
            val vm: EntryEditorViewModel = viewModel(
                key = "editor-$entryId",
                factory = ViewModelFactory.editor(appContainer(context), entryId),
            )
            EntryEditorScreen(
                viewModel = vm,
                imageStorage = imageStorage,
